...
#Parse the configmap for custom cpu and memory resource settings for daemonset and replicaset
/opt/microsoft/omsagent/ruby/bin/ruby omsagent-resource-modifier.rb

#Sourcing environment variable file for enabling/disabling resource set plugin if it exists
if [ -e "enable_resource_set_plugin" ]; then
      cat enable_resource_set_plugin | while read line; do
            echo $line >> ~/.bashrc
      done
      source enable_resource_set_plugin
fi
