...
Prequisites: 
- Powershell version 5.1 or above : To check the powershell version use the command `$psversiontable` and look at the PSVersion row.
- Run powershell as an administrator
- Use 'Get-ExecutionPolicy' to get the current execution policy and store it in a file
- 'Set-ExecutionPolicy' to Unrestricted for running the script

Steps:
- Download TroubleshootError.ps1, ContainerInsightsSolution.json
- You can fetch the subscription id from here : https://blogs.msdn.microsoft.com/mschray/2016/03/18/getting-your-azure-subscription-guid-new-portal/
- Resource group name and AKS Cluster name is in the 'Overview' page of your AKS cluster
...
If the problem remains unsolved. You can use the below mentioned AskCoin id to reach out to us with the TroubleshootDump.txt
